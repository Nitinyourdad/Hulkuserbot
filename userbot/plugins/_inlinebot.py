...
mafia_help_pic = Config.HELP_PIC or "https://telegra.ph/file/852a8de1eaa17e61867c2.jpg"
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query == "@MafiaBot_Support":
            ...
            result = await builder.article(
                ...
                text=f"**Running SurturBot**[⚡🔥]({mafia_help_pic})\n\n__Number of plugins installed__ :`{len(CMD_HELP)}`\n**page:** 1/{veriler[0]}",
                buttons=veriler[1],
                link_preview=True,
            )
            ...
            result = builder.article(
                "@Surturbot_Support",
                text=f"""**Hey![🤗]({mafia_help_pic}) This is [SUrturBot.](https://t.me/surturbot_support)\nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
                        custom.Button.url("🔥 CHANNEL 🔥", "https://t.me/Surturbot_support"),
                        custom.Button.url(
                            "⚡ OWNER⚡", "https://t.me/MAFIARISHABH"
                        ),
                    ],
                    [
                        custom.Button.url(
                            "✨ REPO ✨", "https://github.com/mafiarishabh/SURTURBOT"),
                        custom.Button.url
                    (
                            "🔰 HELP 🔰", "@mafiarishabh"
                    )
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "HELLO THERE. PLEASE MAKE YOUR OWN MAFIABOT AND USE. © surturBot ™",
                cache_time=0,
                alert=True,
            )
        ...
        await event.edit(
            f"**Legenday AF surturBot[⚡🔥]({mafia_help_pic})[.](https://t.me/surturbot_support) __Working...__\n\n**Number of modules installed :** `{len(CMD_HELP)}`\n**page:** {page + 1}/{veriler[0]}",
            buttons=veriler[1],
            link_preview=True,
        )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            await delete_mafia(event,
              f"⚜️sSurturBot Menu Provider Is now Closed[⚜️]({mafia_help_pic})\n\n         **[© SuerturBot ™](t.me/surturbot_support)**[⚡🔥]({mafia_help_pic})", 5, link_preview=True
            )
        else:
            mafia_alert = "HELLO THERE. PLEASE MAKE YOUR OWN SURTURBOT AND USE. © surturBot ™"
    ...
    async def commands(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "HELLO THERE. PLEASE MAKE YOUR OWN SURTURBOT AND USE. © surturBot ™",
                cache_time=0,
                alert=True,
            )
